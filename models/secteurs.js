...
// READ ALL
const findMany = () => {
  return db.query('SELECT * FROM secteurs').then(([result]) => result)
}
...
const findSecteur = ({ nom_secteur }) => {
  ...
  return db.query(sql, [nom_secteur]).then(([results]) => results[0])
}
...
const findBySecteurtWithDifferentId = (secteur, id) => {
  return db
    .query('SELECT * FROM secteurs WHERE nom_secteur = ? AND id_secteur <> ?', [
      secteur,
      id
    ])
