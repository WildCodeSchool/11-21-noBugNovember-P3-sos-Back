...
//READ ALL
const findMany =() => {
    return db.query('SELECT * FROM telechargements').then(([result]) => result)
}
...
//POST ONE
const create = ({ prenom_telechargement, nom_telechargement, mail_telechargement, ville_telechargement, article_id }) => {
   ...
   return db.query(sql,[prenom_telechargement, nom_telechargement, mail_telechargement, ville_telechargement, article_id]).then(([result])=>{
       const id = result.insertId
       return {id, prenom_telechargement, nom_telechargement, mail_telechargement, ville_telechargement, article_id}
   })
...
const update = (id, newAttributes)=>{
    return db.query('UPDATE telechargements SET ? WHERE id_telechargement=?',[newAttributes,id])
}
